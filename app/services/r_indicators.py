    ...
    for doc in documentation:
        if doc.type.lower() not in no_guide_lower:
            if is_url_operational(doc.url):
                logs.append(f"✅ The following documentation is a usage guide and operational: {doc.type} -- {doc.url}")
                operational_user_guide = True
            else:
                logs.append(f"❌ The following documentation is a usage guide but the URL is not operational: {doc.type} -- {doc.url}")
        else:
            logs.append(f"❌ The following documentation is not a usage guide: {doc.type} -- {doc.url}")
...
def compR3_1(instance) -> Tuple[bool, List[str]]:
    ...
    for doc in documentation:
        if doc.type.lower() in contribution_policy_types_lower:
            if is_url_operational(doc.url):
                logs.append(f"✅ A documentation matches contribution policy types and URL is operational: {doc.type} -- {doc.url}")
                operational_contrib = True
            else:
                logs.append(f"❌ A documentation matches contribution policy types but URL is not operational: {doc.type} -- {doc.url}")
        else:
            logs.append(f"❌ A documentation does not match contribution policy types: {doc.type} -- {doc.url}")
    
    if operational_contrib:
        logs.append("✅ At least one documentation matches contribution policy types and URL is operational.")
        logs.append("Result: PASSED")
        return True, logs
    else:
...
def compR4_2(instance) -> Tuple[bool, List[str]]:
    ...
    for doc in documentation:
        if doc.type.lower() in release_policy_types_lower: 
            if is_url_operational(doc.url):
                logs.append(f"✅ A documentation matches release policy types and URL is operational: {doc.type} -- {doc.url}")
                operational_release = True
            else:
                logs.append(f"❌ A documentation matches release policy types but URL is not operational: {doc.type} -- {doc.url}")
        else:
            logs.append(f"❌ A documentation does not match release policy types: {doc.type} -- {doc.url}")

    if operational_release:
        logs.append("✅ At least one documentation matches release policy types and URL is operational.")
        logs.append("Result: PASSED")
        return True, logs
